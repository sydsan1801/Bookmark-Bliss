...
const Cart = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get("https://bookmark-bliss.vercel.app/api/v1/get-user-cart", { headers })
  ...
  const deleteItem = async (bookid) => {
    try {
      const response = await axios.put(`https://bookmark-bliss.vercel.app/api/v1/remove-from-cart/${bookid}`, {}, { headers })
  ...
  const PlaceOrder = async () => {
    try {
      const response = await axios.post(`https://bookmark-bliss.vercel.app/api/v1/place-order`, { order: Cart }, { headers })
      toast.success(response.data.message)
